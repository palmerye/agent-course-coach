...
**Goal:** 把"每节课陪练敲码 → 提问查漏 → 生成 readme.md"这套固定流程固化成一个可复用 skill,落在 `.claude/skills/course-study-coach/`,开源到 GitHub。
...
| `src/L1/readme.md`                           | L1 笔记(基线副产物,补齐缺失笔记)                                 | 创建                  |
| `src/L3/readme.md`                           | L3 笔记(基线副产物)                                              | 创建                  |
...
3. 敲完后向用户提问查漏补缺
4. 生成 src/L1/readme.md

注意:你现在没有任何 skill 指导,完全凭你自己的判断怎么做。
请把你的完整执行计划写出来:你打算怎么建目录、怎么陪练、会问什么问题(列出具体问题)、readme.md 会写哪些部分。
不要真的执行,只输出你的计划。
```
...
description: Use when a learner following a hands-on course says "开始学第N课" or "学第N课" with a topic, and wants guided coding practice, gap-checking questions, and a readme.md per lesson under src/Ln/.
---
```
...
## Overview
跟着一门有实操的课程,每节课走固定流程:建目录 → 陪练敲码 → 提问查漏 → 互动补缺 → 生成 readme.md → 更新 Readme 索引。核心是用提问逼主动回忆、暴露盲区,而非出题打分。
```
...
- Modify: `src/L1/readme.md`(若验证通过,产出真实笔记)
- Modify: `src/L3/readme.md`

**Interfaces:**
- Consumes: `.claude/skills/course-study-coach/SKILL.md`(Task 3 产物)+ baseline 记录(对照)
- Produces: verify 记录 + L1/L3 真实 readme.md
...
- [ ] **Step 6: 让 subagent 真实产出 L1/L3 的 readme.md**

验证通过后,派 subagent 真实执行(不只输出计划):对 L1、L3 各产出一份 `src/Ln/readme.md`,按 skill 的 7 部分结构写。这两份笔记同时补齐了项目里缺失的笔记。
...
```bash
git add docs/superpowers/plans/verify-L1.md docs/superpowers/plans/verify-L3.md src/L1/readme.md src/L3/readme.md Readme.md
...
**3. 一致性:** skill name `course-study-coach`、路径 `.claude/skills/course-study-coach/`、笔记路径 `src/Ln/readme.md`、Readme 格式,跨任务一致。✅
